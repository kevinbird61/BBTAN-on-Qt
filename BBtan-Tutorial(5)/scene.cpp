...
void Scene::Init(int x_start)
{
    ...
    // Setting the sound
    bgm = new QSound("../sound/start.wav");
    ball_hit = new QSound("../sound/ball_hit.wav");
    bgm->play();
}
...
void Scene::DoCollision()
{
    ...
    foreach (QGraphicsItem *i, hit_list) {
        ...
        if(ball->pos().x()>=item->pos().x()+item->size_w || ball->pos().x() <= item->pos().x())
        {
            /* Hit the left or right edge */
            if(ball->pos().y() >= item->pos().y()+item->size_h || ball->pos().y() <= item->pos().y())
            {
            ...
            {
                /* Only Hit the left or right edge */
                ball->x_speed = - ball->x_speed;
                ball->y_speed = ball->y_speed;
            }
            ball->x_speed = - ball->x_speed;
        ...
        {
            /* not Hit the left or right edge */
            if(ball->pos().y() >= item->pos().y()+item->size_h || ball->pos().y() <= item->pos().y())
            {
                /* Hit the bottom or top */
                ball->x_speed = - ball->x_speed;
